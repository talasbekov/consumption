...
def upgrade() -> None:
    ...
    if not op.get_bind().dialect.has_table(op.get_bind(), "status_employees"):
        op.create_table(
            "status_employees",
            sa.Column(
                "status_id",
...
def downgrade() -> None:
    ...
    # Удаляем таблицы с использованием CASCADE
    conn.execute(text("DROP TABLE IF EXISTS users CASCADE"))
    conn.execute(text("DROP TABLE IF EXISTS status_employees CASCADE"))
    conn.execute(text("DROP TABLE IF EXISTS states CASCADE"))
    conn.execute(text("DROP TABLE IF EXISTS employees CASCADE"))
    conn.execute(text("DROP TABLE IF EXISTS divisions CASCADE"))
    conn.execute(text("DROP TABLE IF EXISTS managements CASCADE"))
    ...
    conn.execute(text("DROP TABLE IF EXISTS ranks CASCADE"))
